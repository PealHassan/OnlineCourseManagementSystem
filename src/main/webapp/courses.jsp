...
<%
		...
		if(request.getParameter("Id") != null) {
		    ...
		    String sql = String.format("(select course_id from enrollments where student_id = '%s')",studentId);
		    String sql2 = "select * from courses where course_id not in " + sql;
		    Statement st = con.createStatement();
							...
							<div class="course-info">
								...
								<c:forEach var = "i" begin = "1" end = "<%= signupcnt%>">
									<div class = "text-center pt-5">
										<form method="post" action="CourseDetails.jsp?courseId=<%= URLEncoder.encode(rs.getString("course_id"), "UTF-8")%>&Id=<%= URLEncoder.encode(request.getParameter("Id"), "UTF-8")%>">
										    <input type="submit" value="Enroll Now" class = "act-btn">
										</form>
									</div>
								</c:forEach>
